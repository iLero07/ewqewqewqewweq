...
module.exports = {
    ...
    if(giriscikissystem) {
      ...
      try {
        giriscikiskanal.send({content: `<a:giris:1120626782069076048> ${member} sunucumuza hoşgeldin! Sunucumuz **${member.guild.memberCount}** kişi oldu.`, files: [canvasgiris]});
		} catch(err) { }
        if (member.user.bot) {
        try {
        ...
        if(hgbb) {
            ...
            kanal.send({ embeds: [girismesajs] });
                ...
                .setDescription(`<a:giris:1120626782069076048> | ${member} sunucuya katıldı! Sunucumuz **${member.guild.memberCount}** kişi oldu.`)
				try {
        ...
        if(member.user.bot) {
            if(data) {
                ...
                .setTitle("<:tik:1121054586778681455> | Bot Onaylandı!")
...
try {
            ...
            channel.send({
                content: `<@${member.user.id}>`, embeds: [
                    new Discord.EmbedBuilder()
                        ...
                        .setDescription('• Merhaba Al  kullanıcısı, seni sunucumuza güvenle alabilmek için altta bulunan yazıyı butona tıklayarak yazman gerekiyor.')
        ...
        registerChannel.send({ content: `${member}`, embeds: [
            {
                description: `<a:giris:1120626782069076048> | Seni aramızda görmekten mutluluk duyuyoruz!\n\n<:yapaybot:1121058764003483659> | Sunucumuza katılan **${member.guild.memberCount}.** üyesin.\n\n<:ayarlar:1121055588416237729> | Hesabın <t:${parseInt(member.user.createdTimestamp  / 1000)}:R> açılmıştır.\n\n<:info:1121070942496493698> | Sunucumuzun kurallarını okumayı unutma iyi eğlenceler.`
            }
        ], components: [row] })
    } catch(err) { }
