...
module.exports = {
    ...
    if(giriscikissystem) {
        ...
        giriscikiskanal.send({content: `<a:cks:1120626777044303882> ${member} sunucumuzdan çıktı! Sunucumuz **${member.guild.memberCount}** kişi kaldı.`, files: [canvasgiris]});
        if (member.user.bot) {
        ...
        if(hgbb) {
            ...
            if(sayacmessage) {
                ...
                .setDescription(`<a:cks:1120626777044303882> | ${member} sunucudan ayrıldı! Sunucumuz **${member.guild.memberCount}** kişi kaldı!`)
								try {
    ...
    if(!db.fetch(`rcaptchaOnaylılar_${member.guild.id}`)) return;  
     ...
     db.unpush(`rcaptchaOnaylılar_${member.guild.id}`, member.user.id)  
                    ...
                    const banembed = new EmbedBuilder()
                        .setColor("Red")
                        .setTitle("<:tik:1118497865954771024> | Banlandı!")
