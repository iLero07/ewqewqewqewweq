...
module.exports =  {
  ...
  if (sebep) {
    ...
    .setDescription(`<:soru:1121054584786407464> | Etiketlediğin kullanıcı **${sebep}** sebebiyle afk modunda!`)
  ...
  message.delete()
  message.channel.send(`<:carpi:1121054581888139355> | Hey <@${message.author.id}>, Bu Sunucuda Küfür Engel Sistemi Aktif! `)
  }
  }
  ...
  if(linkler.some(alo => message.content.toLowerCase().includes(alo))) {
    ...
    if(message.author?.bot) return;
  message.delete()
  message.channel.send(`<:carpi:1121054581888139355> | Hey <@${message.author.id}>, Bu Sunucuda Reklam Engel Sistemi Aktif! `)
  }
  }
               ...
               if (message.content == caps) {
                if (!message.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
                   if (!message.mentions.users.first()) {
                    message.delete()
                     const embed = new EmbedBuilder()
                     .setTitle(`<:unlem:1121054576137740318> **UYARI!**`)
